...
function deserializeFast(buffer, i, isArray){
  ...
  while(true) {                           // While we have more left data left keep parsing
    ...
    if(elementType === BSON_DATA_OID) {
      ...
      size = buffer[i] | buffer[i+1] <<8 | buffer[i+2] <<16 | buffer[i+3] <<24;
      object[name] = deserializeFast(buffer, i, false );
      i += size;
    } else if(elementType === BSON_DATA_BINARY) {
      size = buffer[i++] | buffer[i++] << 8 | buffer[i++] << 16 | buffer[i++] << 24;
      var subType = buffer[i++];
      object[name] = new Binary(buffer.slice(i, i += size), subType)
      ...
      size = buffer[i++] | buffer[i++] <<8 | buffer[i++] <<16 | buffer[i++] <<24;
      var code = Utf8ArrayToStr(buffer.slice(i, i += size -1 ));
      object[name] = new Code(code, {});
      i++;
      ...
      var index = i;
			...
			if(index >= buffer.length) throw new Error("Bad BSON Document: illegal CString")
			// Return the C string
      var source = Utf8ArrayToStr(buffer.slice(i, index));
      i = index + 1;
			...
			if(index >= buffer.length) throw new Error("Bad BSON Document: illegal CString")
			// Return the C string
      var regExpOptions = Utf8ArrayToStr(buffer.slice(i, index));
      i = index + 1;
      ...
      // Decode the sizes
      var total = buffer[i++] | buffer[i++] <<8 | buffer[i++] <<16 | buffer[i++] <<24;
      size = buffer[i++] | buffer[i++] <<8 | buffer[i++] <<16 | buffer[i++] <<24;
      var code = Utf8ArrayToStr(buffer.slice(i, i += size -1 ));
      i++;

      // Decode the scope
      var objectSize = buffer[i] | buffer[i + 1] <<8 | buffer[i + 2] <<16 | buffer[i + 3] <<24;
      var scope = deserializeFast(buffer, i, false );
      i+=objectSize;
      object[name] = new Code(code, scope);
    } else {
