  ...
  handle(connection, mongoClient, bson, originalOp, op, liveQueryHandler, options) {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        // Function to execute
        var result = yield mongoClient.db(db).collection(collection).updateOne(EJSON.deserialize(query), EJSON.deserialize(update), commandOptions);
